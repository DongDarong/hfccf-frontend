<script setup>
// Keep the finalized report bundle read-only so guardians can review the
// summary data that Preschool has already finalized.
...
defineOptions({
  name: 'GuardianReportsSummaryPage',
})

const route = useRoute()
const { t } = useI18n()
const studentId = computed(() => String(route.params.studentId || ''))
const { student, reportsSummary, loading, errorMessage } = useGuardianStudentSummary(studentId)
...
<template>
  <GuardianPortalLayout>
    <div class="grid gap-5">
      ...
      <GuardianSummaryCard :title="t('guardianPortal.reports.title')" :subtitle="student?.fullName || t('guardianPortal.reports.subtitle')">
        <pre class="overflow-auto rounded-xl bg-slate-50 p-3 text-xs text-slate-700">{{ formatSummary(reportsSummary?.summary || reportsSummary) }}</pre>
      </GuardianSummaryCard>
