<script setup>
// Keep the schedule summary read-only so guardians can inspect the weekly
// timetable without editing Preschool scheduling data.
...
defineOptions({
  name: 'GuardianScheduleSummaryPage',
})

const route = useRoute()
const { t } = useI18n()
const studentId = computed(() => String(route.params.studentId || ''))
const { student, scheduleSummary, loading, errorMessage } = useGuardianStudentSummary(studentId)
...
<template>
  <GuardianPortalLayout>
    <div class="grid gap-5">
      ...
      <GuardianSummaryCard :title="t('guardianPortal.schedule.title')" :subtitle="student?.fullName || t('guardianPortal.schedule.subtitle')">
        <pre class="overflow-auto rounded-xl bg-slate-50 p-3 text-xs text-slate-700">{{ formatSummary(scheduleSummary?.summary || scheduleSummary) }}</pre>
      </GuardianSummaryCard>
