...
defineOptions({ name: 'PreschoolTeacherAttendanceStudentsPage' })
...
async function loadClasses() {
  try {
    const res = await fetchMyPreschoolClasses({ page: 1, perPage: 100 })
    classOptions.value = (res.items || []).map((c) => ({ label: c.name, value: c.id }))
    if (classOptions.value.length === 1) selectedClassId.value = classOptions.value[0].value
  } catch {
    classOptions.value = []
  }
}
...
<template>
  <MainLayout>
    ...
    <section class="space-y-4">
      <HeaderSection
        :title="t('preschoolTeacherStudentAttendancePage.title')"
        :subtitle="t('preschoolTeacherStudentAttendancePage.subtitle')"
      />
        ...
        <div class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
          ...
          <div class="flex items-end">
            <Button type="button" variant="ghost" size="md" rounded="xl" @click="router.push({ name: 'dashboard-preschool-teacher-attendance' })">
              {{ t('preschoolTeacherStudentAttendancePage.actions.back') }}
            </Button>
          </div>
        </div>
      </div>
